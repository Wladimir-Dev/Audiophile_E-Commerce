...
export const Product = ({ product, girar = false }) => {
  ...
  return (
    <article
      key={product.id}
      className={`${styles.product} ${tablet.product} ${
        product.price && desktop.product
      }`}
    >
      <div
        className={
          (girar && isCategory) || (product.price && !isCategory)
            ? desktop.girar
            : undefined
        }
      >
      ...
      <div
        className={`${styles.container__description} ${
          desktop.container__description
        } ${!product.price && `${styles.centerItems} ${desktop.centerItems}`}`}
      >
        ...
        <span
          className={`
                    ...
                    ${
                      product.price &&
                      `${tablet.textBold} ${desktop.product__name}`
                    }
                    `}
        >
          {product.name}
        ...
        {product.price && !isCategory ? (
          <div className={styles.footer__description}>
            ...
            <div
              className={`${styles.buttonContainer} ${desktop.buttonContainer}`}
            >
              <CounterButton count={counterProduct} fnAux={operation} />
              <button className={`orangeButton`} onClick={handleAddCart}>
                ADD TO CARD
              </button>
            </div>
          </div>
      ...
      {showModal &&
        createPortal(<PopUpItem added={addedRef.current} />, document.body)}
    </article>
  )
}
